...
import {getLineSettingsByLineId} from "../../../../../services/lines/line-settings";
...
const LineUrlSettingModal = ({state, handleClose, data}) => {
    ...
    const {currentTab, onChangeTab} = useTabsNew('ReceiveSetting', 'subActive');
    const {
        isLoading,
        data:lineSetting,
    } = useQuery(['LineSettings', data], () =>
    getLineSettingsByLineId( data),
        {
            cacheTime: 0
        }
    );
    ...
    const ACCOUNT_TABS = [
        {
            ...
            component: <ReceiveSettingForm item={{...lineSetting?.data}} close={handleClose}/>,
            label: 'تنظیمات دریافت'
        },
        {
            value: 'ReceiveStatusSetting',
            icon: <Iconify icon={'ic:round-account-box'} width={20} height={20}/>,
            component: <ReceiveStatusSettingForm item={{...lineSetting?.data}} close={handleClose}/>,
